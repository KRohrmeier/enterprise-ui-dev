...
describe('Kanban Board', () => {
  ...
  it(
    ...
    () => {
      ...
      board.addStatus("Another Thing");

      // Verify that the new status is—in fact—now in board.statuses.
      expect(board.statuses).toContain("Another Thing");
    },
  );
  ...
  it('should remove a status using #removeStatus', async () => {
    ...
    board.removeStatus("Backlog"); // What happens when addStatus is async?? <- use async/await
